INPUT=/tmp/eu-en.gentest.transfer
DEV=`pwd`/bidix
...
cat $INPUT | grep '<v' | grep '@' | cut -f2 -d'@' | cut -f1 -d'<' | lt-proc -w $DEV/../../mt-he.automorf.bin | cut -f2- -d'/' | sed 's/<pres><p3><sg>//g' | grep '<v' | cut -f1 -d'$' | sort -f | uniq -c | sort -gr  | grep -v '[0-9] $' > $DEV/pending_verbs.txt
